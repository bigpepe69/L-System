...
public class JPanelDessinFractal extends JPanel {
	/**
	 * 
	 */
	private static final long serialVersionUID = 5063226157687288154L;
	...
	private float fTailleX;//Variable qui definit la fin du trait suivant l'axe X
	private float fTailleY; //Variable qui definit la fin du trait suivant l'axe Y
	private float fAngleActuel; //Variable qui contient la valeur du dernier angle

	
	// Constructeur
	public JPanelDessinFractal(FenetrePrincipale fenPParam) {
		super();
		fenP = fenPParam;
		fAngle = 90;
		fTailleTrait = 0;
	...
	//Méthode qui ne trace pas le fractal mais fait le calcul pour mettre les coordonnées (en X et en Y) dans un Vector de Vector (le Vector.get(0) contient les coordonnées en X et le Vector.get(1) contient les coordonnées en Y).
	public Vector<Vector<Float>> tracerFictif(float fTailleTraitP){
		...
		Vector<Float> vPositionX = new Vector<Float>(); //Vector qui contiendra toutes les positions des points sur X
		Vector<Float> vPositionY = new Vector<Float>(); //Vector qui contiendra toutes les positions des points sur Y
		...
		for (int i = 0; i < sCurrentSequence.length(); i++) { 
			...
			if (cCurrentCaract == ']') {
				fPositionX=pValeurX.recupElement();
				fPositionY=pValeurY.recupElement();
	...
	//Méthode qui calcule la coordonnée de départ pour que le fractal soit au centre du JPanelDessin (le Vector.get(0) contient la coordonnée X et le Vector.get(1) contient la coordonnée Y)
	public Vector<Float> coordonneDepart(float fEcartXP, float fEcartYP, float fMinXP, float fMinYP, Vector<Vector<Float>> vPositionP){
		...
		if (fMinXP<vPositionP.get(0).get(0)){ //On met le cas ou fMinX est inférieur à notre point de départ à part
		...
		if (fMinYP<vPositionP.get(1).get(0)){ //On met le cas ou fMinY est inférieur à notre point de départ à part
	...
	//Méthode qui permet de calculer la meilleur taille de trait pour que notre fractal soit visible sur au maximum 90% du JPanelDessin
	public float tailleDuTrait(float fEcartXP, float fEcartYP){
		float fTailleTrait=seqMaSequence.getTailleTrait();
		if (((this.getSize().getWidth())-fEcartXP)/fEcartXP<((this.getSize().getHeight())-fEcartYP)/fEcartYP){
			fTailleTrait=(float)(0.9*fTailleTrait*(this.getSize().getWidth())/(fEcartXP));
		}else{
			fTailleTrait=(float)(0.9*fTailleTrait*(this.getSize().getHeight())/(fEcartYP));
		}
		return fTailleTrait;
	}

	
	//Méthode qui permet de zoomer au mieux quand l'utilisateur clique sur Zoom 100%
	public float zoomMieux(float fTailleTraitP){
		...
		vPositions=tracerFictif(fTailleTraitP);
		...
		float fTailleTrait=tailleDuTrait(fEcartX, fEcartY);
		return fTailleTrait;
	}

	
	// Méthode qui trace le Fractal dans le cadre de droite
	public void paintComponent(Graphics g) {
		...
		//On recherche maintenant la valeur du point de départ pour que notre fractal soit au centre du JPanelDessin
		fEcartX=ecartMax(vExtremum.get(0),vExtremum.get(1));
		fEcartY=ecartMax(vExtremum.get(2),vExtremum.get(3));
		...
		//On trace enfin notre fractal final au centre du JPanelDessin
		for (int i = 0; i < sCurrentSequence.length(); i++) { 
			char cCurrentCaract = sCurrentSequence.charAt(i); // Traite chaque caractere un par un

			if (cCurrentCaract == 'F') {
				g.setColor(Color.black);
				g.drawLine(Math.round(fPositionX),Math.round(fPositionY),Math.round(fPositionX+fTailleX), Math.round(fPositionY+fTailleY));				fPositionX=fPositionX+fTailleX;
				fPositionY=fPositionY+fTailleY;
			}
			if (cCurrentCaract == 'G') {
				g.setColor(Color.black);
	...
	//Méthode qui va chercher les paramètres utiles lors des diffèrentes actions sur la fenêtre principale
	public void configVal(){
		seqMaSequence=fenP.getMaSequence();
		fAngle = seqMaSequence.getAngle();
		fTailleTrait = seqMaSequence.getTailleTrait();
		sCurrentSequence = seqMaSequence.getSequenceC().get(seqMaSequence.getIteration());
	}
}
